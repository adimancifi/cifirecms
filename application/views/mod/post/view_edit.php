			...
			<div class="row">
				<div class="col-md-9">
					...
					<div class="form-group mb-4">
						<input type="text" name="title" id="title" class="form-control post-title-input" value="<?=!empty(set_value('title')) ? set_value('title') : $result_post['post_title'];?>" placeholder="<?=lang_line('form_label_title');?>">
						<input id="seotitle" type="hidden" name="seotitle" value="<?=!empty(set_value('seotitle')) ? set_value('seotitle') : $result_post['post_seotitle'];?>" readonly>
					</div>
					<!--/ Title -->
					<!-- Content -->
					<div class="form-group">
						...
						<textarea id="Content" name="content" class="form-control" rows="11" style="border-radius:0px;"><?= !empty(set_value('content')) ? set_value('content') : $result_post['post_content'];?></textarea>
					</div>
					<!--/ Content -->
				</div>
								...
								<div id="collapseStatus" class="collapse show" aria-labelledby="collapse-status" data-parent="#accordionPost">
									<div class="card-body">
										...
										<div class="form-check">
											...
											<label class="form-check-label" for="cHeadlinea"><?=lang_line('form_label_headline');?></label>

											<?php else: ?>

											<input class="form-check-input" type="checkbox" id="cHeadlineb" name="headline" value="1">
											<label class="form-check-label" for="cHeadlineb"><?=lang_line('form_label_headline');?></label>
							...
							<div class="card">
								...
								<div id="collapseCategory" class="collapse" aria-labelledby="collapse-category" data-parent="#accordionPost">
									<div class="card-body">
										...
										<select name="category" class="select-category" data-placeholder="- <?=lang_line('form_label_category');?> -" required>
											<?php
												echo '<option value="'. encrypt($result_post['category_id']) .'">'. $result_post['category_title'] .'</option>';
											?>
										</select>
										<div class="mt-2">
											<small><b><a href="<?=admin_url('category/add-new')?>" target="_blank" class="mt-8"><i class="icon-plus-circle2"></i> <?=lang_line('mod_title_add')?></a></b></small>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="card">
								...
								<div id="collapsedTags" class="collapse" aria-labelledby="collapsed-tags" data-parent="#accordionPost">
									<div class="card-body">
										...
										?>
										<input id="tagsjs" type="text" name="tags" value="<?=$valtag;?>" class="form-control" placeholder="Input Tags">
										<div class="mt-2">
											<small><b><a href="<?=admin_url('tag')?>" target="_blank" class="mt-8"><i class="icon-plus-circle2"></i> <?=lang_line('mod_title_add')?> Tag</a></b></small>
										</div>
									</div>
								</div>
							</div>
								...
								<div id="collapsedAuthor" class="collapse" aria-labelledby="collapsed-author" data-parent="#accordionPost">
									<div class="card-body">
										<select name="author" class="select-2">
											<?php 
												// echo '<option value="'.$result_post['user_id'].'" style="display:none;">'.$result_post['user_name'].'</option>';

												if ( login_level('admin') != 0 && login_level('admin') <= 2 )
												{
													// echo '<option value="'.login_key('admin').'" style="display:none;">'.data_login('admin','name').'</option>';
													foreach ( $all_user as $key_user )
													{
														$selected = ( $result_post['user_id']==$key_user['user_id'] ? 'selected' : '');
														// if ( $key_user['level_id'] == 1 ) continue;
